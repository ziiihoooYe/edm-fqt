...
@click.option('--pkl', 'pkl_name', help='pkl_name of the current model', metavar='STR',             type=str, default=None, required=True)
@click.option('--out_dir', 'out_dir', help='fid result file directory', metavar='STR',              type=str, default=None, required=True)
@click.option('--file_n', 'file_name', help='name of the result file (end with txt)', metavar='STR',  type=str, default='fid_result.txt')

def calc(image_path, ref_path, num_expected, seed, batch, file_name, pkl_name=None, out_dir=None):
    ...
    if dist.get_rank() == 0:
        ...
        # save fid to file
        if (pkl_name and out_dir) is not None:
            fid_file = open(os.path.join(out_dir, file_name), 'at')
            fid_file.write("pkl file name:" + pkl_name + " fid: " + str(fid) + '\n')
            fid_file.flush()
